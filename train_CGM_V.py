...
from model_CGM_V import Encoder_no_ch, Decoder_no_ch, Encoder_vid_embed
...
# ----------------------------------------------------------------------------------------------------------
    ...
    if args.model_type == 's2s':  # seq2seq，不过目前似乎只有这一种type
        ...
        encoder = Encoder_no_ch(frame_size=args.frame_size, frame_embed_size=args.frame_embed_size, hidden_size=args.enc_hid_size,
                          n_layers=1, dropout=0.2).to(device)
        decoder = Decoder_no_ch(frame_embed_size=args.frame_embed_size, word_embed_size=args.wordembed_dim, hidden_size=args.dec_hid_size,
    ...
    dec_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(dec_optimizer, args.epochs, eta_min=0.0002, last_epoch=-1)
    enc_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(enc_optimizer, args.epochs, eta_min=0.0002, last_epoch=-1)
    vid_embed_optimizer_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(vid_embed_optimizer,
                                                                                  args.epochs, eta_min=0.0002, last_epoch=-1)
    ...
    if not eval_only:
        for epoch in range(init_epoch, args.epochs):
            ...
            train_loss, action_test_acc_train = train(train_loader, vid_embed, encoder, decoder, criterion, criterion_action, vid_embed_optimizer, enc_optimizer, dec_optimizer,
            ...
            val_loss, sentbleu, corpbleu, final_results, nltk_result, action_test_acc_val = validate(val_loader, vid_embed, encoder, decoder, criterion, criterion_action, tok_tgt, result_path, epoch)  # 一个验证周期，函数在下面
    ...
    eval(test_loader, vid_embed, encoder, decoder, cp_file, tok_tgt, result_path)


def train(train_loader, vid_embed, encoder, decoder, criterion, criterion_action, vid_embed_optimizer, enc_optimizer, dec_optimizer, epoch, EPOCH_NUMBER):
    '''
    执行每个eopch的训练
    '''
    vid_embed.train()
    ...
    for cnt, (srccap, tgtcap, video, img_key, img_key_neg, caplen_src, caplen_tgt, srcrefs, tgtrefs, action_class_label, srccap_half, caplen_src_half) in enumerate(train_loader, 1):
        ...
        init_hidden, vid_out_rnn, act_class = encoder(video)
        ...
        scores, contrastive_attention = decoder(tgtcap, init_hidden, vid_out, vid_out_rnn, args.MAX_INPUT_LENGTH,
...
def validate(val_loader, vid_embed, encoder, decoder, criterion, criterion_action, tok_tgt, result_path, epoch):
    '''
    ...
    with torch.no_grad():
        ...
        for cnt, (srccap, tgtcap, video, img_key, img_key_neg, caplen_src, caplen_tgt, srcrefs, tgtrefs, action_class_label, srccap_half, caplen_src_half) in enumerate(val_loader, 1):
            ...
            scores, pred_lengths = decoder.inference(tgtcap, init_hidden, vid_out, vid_out_rnn, args.MAX_INPUT_LENGTH)
...
def eval(test_loader, vid_embed, encoder, decoder, cp_file, tok_tgt, result_path):
    '''
    测试模型
    '''
    ### 会用最好的模型来测试
    ...
    with torch.no_grad():
        ...
        for cnt, (srccap, tgtcap, video, img_key, img_key_neg, caplen_src, caplen_tgt, srcrefs, tgtrefs, action_class_label, srccap_half, caplen_src_half) in enumerate(test_loader, 1):
            ...
            preds, pred_lengths = decoder.beam_decoding(init_hidden, vid_out, vid_out_rnn, args.MAX_INPUT_LENGTH,
                                                        width=1)
